...
def save_tensor(name, step, x, num):
    mean = x.mean(dim=[0, 2, 3], keepdim=False)
    std = (x.var(dim=[0, 2, 3], keepdim=False) + 1e-6).sqrt()
    mean = np.array(mean.detach().cpu())
    std = np.array(std.detach().cpu())
    path = '/home/chenming/DomainBed_raw/domainbed_ccfp/save_res/our/mean' + str(num) + '/' 
    if not os.path.exists(path):
        os.makedirs(path)
    path = path + str(name) + str(step)
    np.savetxt(path,mean)
    path = '/home/chenming/DomainBed_raw/domainbed_ccfp/save_res/our/std' + str(num) + '/'
    if not os.path.exists(path):
        os.makedirs(path)
    path = path + str(name) + str(step)
    np.savetxt(path,std)
...
def accuracy(network, loader, weights, device, name, step):
    correct = 0
    total = 0
    weights_offset = 0
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            y = y.to(device)
            p = network.predict(x)
            # save_tensor(name, step, network.id_featurizer_tea.x2, 2)
            # save_tensor(name, step, network.id_featurizer_tea.x3, 3)
            # save_tensor(name, step, network.id_featurizer_tea.x2, 2)
            # save_tensor(name, step, network.id_featurizer_tea.x3, 3)
