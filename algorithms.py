...
from resnet_mixstyle import resnet50_mixstyle_L234_p0d5_a0d1
import random
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, device='cpu'):
        ...
        # self.scheduler = lr_scheduler.MultiStepLR(self.optimizer, milestones=[4000,7000] ,gamma=0.1)

    def update(self, minibatches, test_env, unlabeled=None):
...
class Mixstyle_raw(Algorithm):
    ...
    def update(self, minibatches, test_env, **kwargs):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        # all_x = torch.cat(x)
        # all_y = torch.cat(y)
...
#dual
        self.featurizer_tea = networks.Featurizer(input_shape, self.hparams, 1)

        self.classifier_tea = networks.Classifier(
            self.featurizer.n_outputs,
            num_classes,
            self.hparams['nonlinear_classifier'])
        ...
        self.optimizer_tea = torch.optim.Adam(
            self.network_tea.parameters(),
            lr=self.hparams["lr"],
            weight_decay=self.hparams['weight_decay']
        )
    ...
    def update(self, minibatches, test_env, **kwargs):
        ...
        # student
        fea_x = self.featurizer(all_x)
        predict_x = self.classifier(fea_x)
        self.loss_id = self.id_criterion(predict_x, all_y)

        # teacher
        fea_x_tea = self.featurizer_tea(all_x)
        predict_x_tea = self.classifier_tea(fea_x_tea)
        self.loss_id_tea = self.id_criterion(predict_x_tea, all_y)

        self.consis_stu = self.E_dis(predict_x, predict_x_tea.detach())
        self.consis_tea = self.E_dis(predict_x.detach(), predict_x_tea)

        self.loss_stu = self.loss_id + self.consis_stu
        self.loss_tea = self.loss_id_tea + self.consis_tea

        self.optimizer.zero_grad()
        self.loss_stu.backward()
        self.optimizer.step()

        self.optimizer_tea.zero_grad()
        self.loss_tea.backward()
        self.optimizer_tea.step()

        return {"loss": self.loss_stu.item()}

    def predict(self, x):
        x0 = self.featurizer(x)
...
class Mixstyle2(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, device='cpu'):
        assert input_shape[1:3] == (224, 224), "Mixstyle support R18 and R50 only"
        super().__init__(input_shape, num_classes, num_domains, hparams, device)
        if hparams["resnet18"]:
...
class CCFP(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, device='cpu'):
        super(CCFP, self).__init__(input_shape, num_classes, num_domains, hparams, device)
        self.hparams = hparams
        self.iteration = 0
        ...
        self.network_tea = nn.Sequential(self.featurizer_tea, self.classifier_tea)
        self.id_featurizer_tea = self.featurizer_tea
        self.dis_id_tea = self.classifier_tea
        ...
        self.optimizer_LD = torch.optim.Adam([
            {'params': self.network_tea[0].norm0.parameters()},
            {'params': self.network_tea[0].norm1.parameters()},
            {'params': self.network_tea[0].norm2.parameters()},
            {'params': self.network_tea[0].norm3.parameters()},
            {'params': self.network_tea[0].norm4.parameters()}],
            lr=lr,
        )
    ...
    def forward(self, x, y):
        ...
        #randomly apply perturbation
        # perturbation = random.random() < 1.
        # if perturbation:
        #     perturb = True
        # else:
        #     perturb = False
        perturb = True
        # student
        fea_x = self.id_featurizer(x)
        predict_x = self.dis_id(fea_x)
        self.loss_id = self.id_criterion(predict_x, y)

        # teacher
        fea_x_tea = self.id_featurizer_tea(x, perturb)
        predict_x_tea = self.dis_id_tea(fea_x_tea)
        self.loss_id_tea = self.id_criterion(predict_x_tea, y)
        ...
        # self.loss_stu = self.loss_id
        # self.loss_tea = self.loss_id_tea
        ...
        if self.hparams['MNIST_CNN']:
            # student
            _ = self.id_featurizer(x)
            fea_0, fea_1, fea_2 = self.id_featurizer.x0, self.id_featurizer.x1, self.id_featurizer.x2

            # teacher
            _ = self.id_featurizer_tea(x, perturb)
            fea_tea_0, fea_tea_1, fea_tea_2 = self.id_featurizer_tea.x0, self.id_featurizer_tea.x1, self.id_featurizer_tea.x2
            ...
            self.loss_norm = self.hparams['lambda_dis'] * (self.simi_tea0 + self.simi_tea1 + self.simi_tea2)
        else:
            # student
            _ = self.id_featurizer(x)
            ...
            self.loss_norm = self.hparams['lambda_dis'] * (self.simi_tea0 + self.simi_tea1 + self.simi_tea2 + self.simi_tea3 + self.simi_tea4)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {
            'loss_id': self.loss_id.item(),
            'loss_id_tea': self.loss_id_tea.item(),
            'loss_stu': self.consis_stu.item(),
            'loss_tea': self.consis_tea.item(),
        }

    def predict(self, x):
        x0 = self.id_featurizer_tea(x, True)
        x0 = self.dis_id_tea(x0)
        # x1 = self.id_featurizer(x)
        # x1 = self.dis_id(x1)
        # x = (x0 + x1) / 2
        return x0
